...
Joystick_Status_Enum Joystick_Get_Status(void) {
    ...
    if (!(joystick_state & (1 << 24))) return JOYSTICK_UP;
    if (!(joystick_state & (1 << 26))) return JOYSTICK_DOWN;
    if (!(joystick_state & (1 << 23))) return JOYSTICK_LEFT;
    if (!(joystick_state & (1 << 25))) return JOYSTICK_RIGHT;
		if (!(joystick_state & (1 << 20))) return JOYSTICK_CENTER;
}