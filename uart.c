...
void UART_Initialize(unsigned int baudRate)
{
  ...
  LPC_SC->PCONP |= 1<<3;
	...
	LPC_PINCON->PINSEL0 &= ~0x000000F0;  // each pin wil contain 2 Pinsel ex. Pin0 contain pinsel.0 and .1
  ...
  // Enable UART0 interrupt
	LPC_UART0->IER = IER_EnableRBR | IER_EnableTHRE | IER_EnableRLS; // IER turn onUart, THRE set when UART is cleared, RLS recieve line status
